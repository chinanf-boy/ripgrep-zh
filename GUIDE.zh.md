...
除了，跳过隐藏文件和在`.gitignore`内的文件,
ripgrep 还会跳过 二进制文件。 ripgrep 默认这么做，是因为
...
2. 二进制模式与默认模式类似, 除了在发现`NUL`字节之后，并不总是停止搜索。 换句话说, 在这个模式下, ripgrep
...
3. 文本模式完全禁用，所有的二进制检查，和若是文件是文本，就搜索全部。对主要是文本，但也包括`NUL`字节的文件来说，很有用，或者如果你特别想尝试搜索下二进制数据。 用 `-a/--text`标志启动该模式。
...
| `-c/--count`       | 报告总匹配行的计数。                                                                                                                           |
| `--files`          | 打印 ripgrip *将*搜索的文件，但不要实际搜索它们。                                                                                              |
